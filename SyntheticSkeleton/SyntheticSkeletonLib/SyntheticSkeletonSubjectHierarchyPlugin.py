...
class SyntheticSkeletonSubjectHierarchyPlugin(AbstractScriptedSubjectHierarchyPlugin):

  # Necessary static member to be able to set python source to scripted subject hierarchy plugin
  filePath = __file__

  def __init__(self, scriptedPlugin):
    scriptedPlugin.name = 'SyntheticSkeleton'
  ...
  def canAddNodeToSubjectHierarchy(self, node, parentItemID = None):
    if node is not None and node.IsA("vtkMRMLScriptedModuleNode"):
      if node.GetAttribute("ModuleName") == "SyntheticSkeleton":
        return 0.9
    return 0.0
  ...
  def icon(self, itemID):
    ...
    if associatedNode is not None and associatedNode.IsA("vtkMRMLScriptedModuleNode"):
      if associatedNode.GetAttribute("ModuleName") == "SyntheticSkeleton" and \
          associatedNode.GetAttribute("Type") == "SyntheticSkeletonNode":
        iconPath = os.path.join(os.path.dirname(__file__), '../Resources/Icons/SyntheticSkeleton.png')
      elif associatedNode.GetAttribute("ModuleName") == "SyntheticSkeleton" and \
          associatedNode.GetAttribute("Type") == "TriangleLabel":
        iconPath = os.path.join(os.path.dirname(__file__), '../Resources/Icons/triangle-icon.png')
